export default function Home() {
  return (
    <div>
      <div className="flex items-center justify-center min-h-screen bg-white text-black">
        <div className="text-center rounded-3xl p-20">
          <div className="max-w-md">
            <h1 className="text-5xl font-bold">All your finances in one place</h1>
            <p className="py-6">
              AppName is currently in beta, new features will be coming soon, such as direct integration with crypto wallets, exchanges, and stock brokerages.
            </p>
            <a className="inline-block px-6 py-3 bg-action-primary text-white font-semibold rounded-lg hover:opacity-90 transition-opacity" href="/portfolio">
              Get Started
            </a>
          </div>
        </div>
      </div>
    </div>
  )
}
